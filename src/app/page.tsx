    ...
    <div className="max-w-7xl mx-auto content-padding">
            ...
            <div className="flex-1 p-4 sm:p-5 flex flex-col gap-4">
              <div className="flex-1 min-w-0">
                    ...
                    {isDiscountActive(house.discount) ? (
                      <>
                        ...
                        <p className="text-lg font-bold text-red-600">
                          ...
                          <span className="text-sm font-normal text-[var(--muted)]">/{t('day', 'хоног')}</span>
                        </p>
                      </>
                    ) : (
                ...
                <Link href={`/houses/${house.id}`} className="block mt-1">
                  ...
                  <p className="mt-2 text-sm text-[var(--muted)] line-clamp-2">{house.localizedDescriptions?.[currentLanguage] || house.description}</p>
                </Link>
              </div>
              <div className="flex items-center justify-between gap-2 flex-wrap text-sm text-[var(--muted)]">
                <div className="flex items-center">
                  <Users size={16} className="mr-1.5" />
                  {house.capacity} {t('people', 'хүн')}
                </div>
              </div>
              <Link
      ...
      {houses.length === 0 && (
        <div className="card text-center py-12 px-4">
          <p className="text-[var(--muted)]">{t('no_houses_available', 'Одоогоор боломжтой байшин алга байна.')}</p>
        </div>
      )}
...
function MapSection({ settings }: { settings: any }) {
  ...
  return (
    <section className="mt-12 sm:mt-16 lg:mt-20">
      {settings?.map && (
        <>
          <h2 className="text-xl sm:text-2xl font-bold text-[var(--foreground)] mb-6 sm:mb-8 text-center">{t('our_location', 'Бидний Байршил')}</h2>
      ...
      <footer className="mt-12 sm:mt-16 border-t border-[var(--border)] pt-10 pb-10 safe-bottom">
        <div className="max-w-2xl mx-auto text-center">
          {(settings?.social?.facebook || settings?.social?.instagram) && (
            <div className="flex justify-center gap-6 mb-6">
              {settings.social.facebook && (
                <a href={settings.social.facebook} target="_blank" rel="noopener noreferrer" className="text-[var(--muted)] hover:text-[var(--primary)] transition-colors touch-target flex items-center justify-center" aria-label={t('facebook', 'Facebook')}>
              ...
              {settings.social.instagram && (
                <a href={settings.social.instagram} target="_blank" rel="noopener noreferrer" className="text-[var(--muted)] hover:text-[var(--primary)] transition-colors touch-target flex items-center justify-center" aria-label={t('instagram', 'Instagram')}>
          ...
          <div className="mb-10 pt-8 border-t border-[var(--border)]">
            <h3 className="text-sm font-bold text-[var(--foreground)] uppercase tracking-wider mb-4">{t('choose_language', 'Хэл сонгох')}</h3>
            <div className="flex flex-wrap justify-center gap-3">
              {languages.map((lang) => (
                <button
                  key={lang.id}
                  onClick={() => setLanguage(lang.id)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all ${currentLanguage === lang.id
          ...
          <p className="text-sm text-[var(--muted)]">
            © {new Date().getFullYear()} {settings?.branding?.siteName || "ResortBook"}. {t('footer_rights', 'Бүх эрх хуулиар хамгаалагдсан.')}
          </p>
          <a href="/admin/login" className="inline-block mt-2 text-xs text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors">
            {t('footer_admin_login', 'Админ нэвтрэх')}
          </a>
        </div>
      </footer>
    </section>
